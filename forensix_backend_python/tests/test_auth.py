...
# ── SEC-07 password-length tests ─────────────────────────────────────────────

def test_register_password_too_short_rejected(client):
    """SEC-07: Password below minimum length (< 8) must be rejected."""
    resp = client.post("/api/auth/register", json={
        "full_name": "Short Pass User",
        "username": "shortpass_user",
        "email": "shortpass@example.com",
        "password": "abc1234",   # 7 chars — below minimum
        "role": "citizen",
    })
    assert resp.status_code == 422, resp.text
    body = resp.json()
    # Pydantic returns detail list with validation errors
    assert any("8" in str(err) or "characters" in str(err).lower() for err in body.get("detail", []))


def test_register_password_at_minimum_accepted(client):
    """SEC-07: Password at exactly the minimum length (8) must be accepted."""
    resp = client.post("/api/auth/register", json={
        "full_name": "Min Pass User",
        "username": "minpass_user",
        "email": "minpass@example.com",
        "password": "abcd1234",   # exactly 8 chars — meets minimum
        "role": "citizen",
    })
    assert resp.status_code == 200, resp.text
    assert resp.json()["username"] == "minpass_user"
